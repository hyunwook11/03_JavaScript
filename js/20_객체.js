...
btn1.addEventListener("click", () =>(){
    ...
    // 1) 수정 - key가 중보고되면 새로운 value로 덮어쓰기
    // 객체.기존key = 새 value;
    // 객체 ["기존key"] = 새 value;
...
btn2.addEventListener("click", () => {
    ...
    student.introduce = function(){
        ...
        // 저는 20세 남자 홍길동 입니다
        alert(`저는 ${this.age}세 ${this.gender} ${this.name}입니다`);
...
btn.onclick = function(){ consloe.log(this);}
...
function Student(grade, ban, number, name){
    ...
    // 생성된 Studuent 객체의 key가 grade인 필드에
...
const studentList = [
    ...
    new student(1, 1, 2, "홍길동"),
    new student(1, 1, 3, "홍길동"),
];
...
addStudent.addEventListener("click", () => {
    ...
    const std = new Student(
        Number(values[1]), Number(values[2]), Number(values[3]), values[3]
    );
...
selectStudent.addEventListener("click", () => {
    ...
    tbody.innerHTML = ""; //tboby 내부 내용 모두 삭제
    ...
    //studentList를 반복 접근 하면서 tr, th, td 요소
    //만들어서 추가
    studentList.forEach( (std, index) => {
        ...
        // d이름이 작성된 요소(td1)가 클릭 되었을 때

        td1.addEventListener("click", () => {
            alert(std.inform()); // inform 메서드 반환 결과 alert
        });
    });
...
/* 배열.forEach ((item, index) => {})
    ...
    // }); */
const checkJson = document.querySelector("#checkKson");
checkJson.addEventListener("click", () => {
    ...
    const menu = '{"name" : "김밥", "price" : 40000}';
    console.log("menu : ", menu, typeof menu);

    //JSON.parse(JSON 문자열) : JSON문자열 -> JS 객체
