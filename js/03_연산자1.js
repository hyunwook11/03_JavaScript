...
function plusFn(){
    ...
    // 단, '숫자 형태 문자열'이 아닐 경우 NaN(Not a Number) 결과 반환
...
function minusFn(){
    ...
    -> input에 작성된 값이 없어서number (input1.value == 0

        해결방법
        값을 입력한ㅁ 후 함수를 호출해서 함수가 호출되는
        시점의 입력된 값을 읽어올 수 있게 만든다 */
...
function doubleFb(){
    ...
    calcResult2.innerText = value2 * 2;
}
function squareFn(){
    const value4 = Number(input4);
...
function increase(){
    ...
    //요소.innerText = "문자열"; //요소에 작성된 내용 얻어오기
...
// ex) +,-,*,/,%,++,--,
//변 수에 값 대입, console 출력, innerText ... 모두 다
function checkFn(){
    let num = 100;
