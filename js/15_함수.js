...
const inputs2 = document.querySelectorAll("[name = input3]");
...
function pow(num, x){
    ...
    let result = 1;
    for(let i = 0; i<x ; i++){
        result *= num; // result에 num을 계속 곱홤
    }
    return result; // 함수 종료 + 호출한 곳으로 result 가지고 돌아감
}

/**  제곱의 결과를 문자열로 변환ㄴ해서 반환하는 함수
 * @param num
 * @param x
 * @param result : num의 제곱
 * @returns "num의 x제곱은 result입니다"
*/
...
function createArray(num){
    ...
    for (let i=0 ; i<num ; i ++){
        Number(prompt(`${i}번째 인덱스에 대입할 숫자`))
    }
    arr.push(val) //배열 마지막 요소로 val 추가
    return arr;
}

const btns = document.querySelector("#btn5");
btn5.addEventListener("click", function(){

    const arr =createArray(5); // 생성된 배열
...
document.querySelector("#btn9").addEventListener("click", () => {
    ...
    - 창(브라우저) 관련 가능을 제공(alert, prompt, setTimeout,
      setInterval, open 등..)
     */
