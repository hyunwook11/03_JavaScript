...
/**1부터 7까지 1씩 증가한 문자열을 한 줄로 출력 */
function check4(){
    let result =""; //결과를 저장할 변수에 "" (빈칸) 대입

    for(let num = 1; num <= 7 ; num ++){
        result += num; //result = reslt + num;
    }
...
// check4, 5와
function check6(){
    let sun = 0; //합계를 저장할 변수를 선언, 0을 대입
...
function check12(){
    const A = Number(document.getElementById("dan12").value);
    if(A.value.length == 0){
        alert("단을 입력해주세요");
        return;
    }
...
/** 5의 배수마다 @abstract, 10의 배수마다 줄바꿈하면서 출력하기(1~30) */
function check16(){
    let result = 0;
...
function check26(){
    ...
    console.log(`1~100 사이 ${input}의 배수는 ${num}개 있습니다`)
...
function check29(){
    ...
    while(input !== null){
        ...
        input = prompt("메뉴 번호 입력(1.김밥 2.라면 3.돈가스)")
