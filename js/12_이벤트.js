...
testB.onclick = function(){
    ...
    //testA가 클릭 되었을 때의 동적(핸들러)을 null로 덮어씌움
    // -> 이벤트 핸들러가 살라짐
    testA.onclick = null;
...
const bg = document.querySelectorAll(".inputColor");
const input = document.querySelector(".display")


for (let i = 0 ; i < bg.value ; i++){

    change.addEventListener("click", function(){
        input.style.backgroundColor = bg[i].value;
    })
}
// 변경하기 버튼 클릭 시
//inout 요소애 작성된 색상 값을 얻어와
